...
build_download_url() {
    ...
    echo "https://github.com/lyqray/flux-panel/releases/download/1.4.3B/gost-${ARCH}"
}
...
install_flux_gost3() {
  ...
  mkdir -p "$INSTALL_DIR"
  echo "⬇️ 下载执行文件..."
  curl -L "$DOWNLOAD_URL" -o "$INSTALL_DIR/flux_gost3"
  if [[ ! -s "$INSTALL_DIR/flux_gost3" ]]; then
    echo "❌ 下载失败。"
    exit 1
  fi
  chmod +x "$INSTALL_DIR/flux_gost3"
...
services:
  flux_gost3:
    ...
    volumes:
      - ./:/etc/gost
    command: ["/etc/gost/flux_gost3", "-C", "/etc/gost/config.json", "-C", "/etc/gost/gost.json"]
EOF
...
update_flux_gost3() {
  ...
  echo "🛑 停止当前容器..."
  cd "$INSTALL_DIR" && $DOCKER_CMD stop flux_gost3 2>/dev/null

  echo "⬇️ 下载最新二进制文件..."
  curl -L "$DOWNLOAD_URL" -o "$INSTALL_DIR/flux_gost3"
  if [[ ! -s "$INSTALL_DIR/flux_gost3" ]]; then
    echo "❌ 下载失败。"
    $DOCKER_CMD start flux_gost3 # 失败了要把旧的拉起来
    exit 1
  fi
  chmod +x "$INSTALL_DIR/flux_gost3"
