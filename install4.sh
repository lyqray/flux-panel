...
install_flux_gost4() {
  ...
  mkdir -p "$INSTALL_DIR"
  echo "⬇️ 下载执行文件..."
  curl -L "$DOWNLOAD_URL" -o "$INSTALL_DIR/flux_gost4"
  if [[ ! -s "$INSTALL_DIR/flux_gost4" ]]; then
    echo "❌ 下载失败。"
    exit 1
  fi
  chmod +x "$INSTALL_DIR/flux_gost4"
...
services:
  flux_gost4:
    ...
    volumes:
      - ./:/etc/gost
    command: ["/etc/gost/flux_gost4", "-C", "/etc/gost/config.json", "-C", "/etc/gost/gost.json"]
EOF
...
update_flux_gost4() {
  ...
  echo "🛑 停止当前容器..."
  cd "$INSTALL_DIR" && $DOCKER_CMD stop flux_gost4 2>/dev/null

  echo "⬇️ 下载最新二进制文件..."
  curl -L "$DOWNLOAD_URL" -o "$INSTALL_DIR/flux_gost4"
  if [[ ! -s "$INSTALL_DIR/flux_gost4" ]]; then
    echo "❌ 下载失败。"
    $DOCKER_CMD start flux_gost4 # 失败了要把旧的拉起来
    exit 1
  fi
  chmod +x "$INSTALL_DIR/flux_gost4"
